...
def load_schema():
    ...
    # Auto-generate config directory and master template if missing
...
try:
    ...
    col2.metric(
        "Gemini Extraction Target Fields", len(schema.extraction_fields)
    )
    ...
    for header in schema.master_headers:
        ...
        table_data.append({
            ...
            "Gemini Prompt Status": (
                "Excluded from Prompt" if is_reserved else "Active JSON Key"
            ),
        })
